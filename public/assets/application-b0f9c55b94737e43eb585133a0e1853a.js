...
(function() {
  ...
  find_book = function(isbn) {
    var content, url;
    content = $('#content');
    if (isbn !== "" && isbn.length === 13 && parseInt(isbn, 10)) {
      url = "https://www.googleapis.com/books/v1/volumes?q=isbn:" + isbn + "&key=AIzaSyDqJ6dEoT_kIbfBSV8ztbZOqzZCqRRQtQc";
      console.log(url);
      return $.get(url, function(data) {
        var book, book_div, description, img, img_div, img_link, info_div, link;
        if (data.totalItems !== 0) {
          ...
          return book_div.insertAfter($('#search_link'));
        } else {
          $('<div class="error">').html("Could not find book").insertAfter($('#search_link'));
          return setTimeout((function() {
            return $('.error').fadeOut(1000, function() {
              return $('.error').remove();
            });
          }), 1000);
        }
